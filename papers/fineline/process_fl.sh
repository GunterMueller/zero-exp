...
for d in $EXPDIR/$EXPNAME-*; do
    ...
    if [ ! -f $DIR/agglog.txt ]; then
        cp $DIR/tput.txt $DIR/agglog.txt
    fi
    ...
    awk -v title="$TITLE" -v window=$MAVG_WINDOW '
         ...
         $1 > 0 { s += $1; c++ }
         END { print s/c }' \
        $DIR/agglog.txt > $DIR/avgtput.txt
    ...
    touch $EXPDIR/avgtput.txt
    paste $DIR/avgtput.txt $EXPDIR/avgtput.txt > $EXPDIR/tmp.txt
    mv $EXPDIR/tmp.txt $EXPDIR/avgtput.txt
